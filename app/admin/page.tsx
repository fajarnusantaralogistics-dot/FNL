...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    (function patchHistoryOnce() {
      if (typeof window === "undefined" || (window as any).__patchedHistory)
        return;
  ...
  const [clientCategory, setClientCategory] = useState<
    "all" | "corporate" | "otomotif" | "property"
  >("all");
  const [clientFormCategory, setClientFormCategory] = useState<
    "corporate" | "otomotif" | "property"
  >("corporate");
  ...
  async function loadClients() {
    const q =
      clientCategory && clientCategory !== "all"
        ? `?category=${encodeURIComponent(clientCategory)}`
        : "";
  ...
  // overview stats
  const [overview, setOverview] = useState<{ clients: number; images: number }>(
    { clients: 0, images: 0 },
  );
  ...
  async function uploadClient(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok)
        setAlert({ type: "success", text: "Client berhasil ditambahkan" });
      else
        setAlert({
          type: "error",
          text: data?.error || "Gagal menambahkan client",
        });
    } catch (e: any) {
  ...
  async function uploadTestimonial(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok)
        setAlert({ type: "success", text: "Testimonial berhasil ditambahkan" });
      else
        setAlert({
          type: "error",
          text: data?.error || "Gagal menambahkan testimonial",
        });
    } catch (e: any) {
      setAlert({ type: "error", text: e?.message || "Network error" });
    } finally {
  ...
  async function removeClient(logoPublic: string) {
    ...
    await fetch("/api/clients", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ logoPublic }),
    });
    await loadClients();
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <AdminToast message={alert} />

      <h1 className="text-2xl font-bold mb-4">
        Admin - Kelola Gambar & Client
      </h1>

      <div className="mb-4">
        {alert ? (
          <div
            className={`p-3 rounded ${alert.type === "success" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
          >
            {alert.text}
          </div>
        ) : null}
      ...
      {section === "overview" && (
        <OverviewPanel overview={overview} recentClients={clients} />
      )}
      {section === "clients" && (
        <ClientsPanel
          clients={clients}
